...
from eegda.config import kwargs_to_argv, load_config
from eegda.datasets.base import EEGDataset
from eegda.datasets.standard_npz import StandardDataset
from eegda.engine import run_backend
from eegda.results import EEGDAResults
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="EEGDA training CLI")
    parser.add_argument("--config", default=None)
    parser.add_argument(
        "--data",
        default=None,
        help="Path to an EEGDA-standard .npz dataset. This is the recommended user-facing dataset input.",
    )
...
def main(argv: list[str] | None = None) -> EEGDAResults:
...
def run_from_kwargs(**kwargs) -> EEGDAResults:
    ...
    if isinstance(dataset_obj, EEGDataset):
        ...
        if isinstance(standard_dataset, StandardDataset) and standard_dataset.path is not None:
            ...
            return main(kwargs_to_argv(kwargs))
        with TemporaryDirectory(prefix="eegda_") as tmpdir:
...
def build_result(args: argparse.Namespace, backend_argv: list[str]) -> EEGDAResults:
    label_protocol = label_protocol_for_args(args)
    dataset_name = effective_dataset(args)
    dataset_key = dataset_key_for_args(args)
    output_dir = _resolve_output_dir(args.output_dir, dataset_key, args.model, args.method, label_protocol)
